...
class newenvironment():
    ...
    def register(self):
        ...
        if self.driver.find_element_by_id(self.element_json[u'首页注册ID'])!=[]:
            os.popen('adb -s %s shell input tap %s %s' % (self.deviceid,self.coordinate.randint(self.zb['HomeRegistration']["x"]['value'],self.zb['HomeRegistration']["x"]['change']),self.coordinate.randint(self.zb['HomeRegistration']["y"]['value'],self.zb['HomeRegistration']["y"]['change'])))
        time.sleep(1)
    ...
    def input_text(self):
        ...
        if self.driver.find_elements_by_id(self.element_json[u'输入框ID'])!=[]:
            os.system('adb -s %s shell input tap %s %s' % (self.deviceid, self.coordinate.randint(self.zb['InputName']["x"]['value'], self.zb['InputName']["x"]['change']),self.coordinate.randint(self.zb['InputName']["y"]['value'], self.zb['InputName']['y']['change'])))
        time.sleep(3)
        for i in  list(self.wxmc):
            time.sleep(0.3)
            os.system('adb -s %s shell input tap %s %s'%(self.deviceid,self.coordinate.randint(self.zb[i]["x"]['value'],self.zb[i]["x"]['change']),self.coordinate.randint(self.zb[i]["y"]['value'],self.zb[i]['y']['change'])))
        logging.info(self.deviceid + u'-输入昵称')
        #self.driver.find_elements_by_id(self.element_json[u'输入框ID'])[1].clear()
        os.system('adb -s %s shell input tap %s %s' % (self.deviceid, self.coordinate.randint(self.zb['InputPhoneNumber']["x"]['value'], self.zb['InputPhoneNumber']["x"]['change']),self.coordinate.randint(self.zb['InputPhoneNumber']["y"]['value'], self.zb['InputPhoneNumber']['y']['change'])))
        ...
        time.sleep(random.randint(1, 3))
        self.Judgment_Devices()
        os.system('adb -s %s shell input tap %s %s' % (self.deviceid, self.coordinate.randint(self.zb['InputPassWord']["x"]['value'], self.zb['InputPassWord']["x"]['change']),self.coordinate.randint(self.zb['InputPassWord']["y"]['value'],self.zb['InputPassWord']['y']['change'])))
        time.sleep(random.randint(1, 3))
        for i in  list(self.wxmm):
            time.sleep(0.3)
            os.system('adb -s %s shell input tap %s %s' % (self.deviceid, self.coordinate.randint(self.zb[i]["x"]['value'], self.zb[i]["x"]['change']),self.coordinate.randint(self.zb[i]["y"]['value'], self.zb[i]['y']['change'])))
            if i == 'a':
                os.system('adb -s %s shell input tap %s %s' % (self.deviceid, self.coordinate.randint(self.zb['SwitchNo']["x"]['value'], self.zb['SwitchNo']["x"]['change']),self.coordinate.randint(self.zb['SwitchNo']["y"]['value'], self.zb['SwitchNo']['y']['change'])))
                time.sleep(1)
        ...
        time.sleep(random.randint(1, 3))
        self.Judgment_Devices()
        os.system('adb -s %s shell input tap %s %s' % (self.deviceid,self.coordinate.randint(self.zb['InputRegistration']["x"]['value'], self.zb['InputRegistration']["x"]['change']),self.coordinate.randint(self.zb['InputRegistration']["y"]['value'], self.zb['InputRegistration']['y']['change'])))
        ...
        while True:
            ...
            if self.driver.find_elements_by_class_name(self.element_json['CheckBox']) != []:
                os.system('adb -s %s shell input tap %s %s' % (self.deviceid, self.coordinate.randint(self.zb['CheckBox']["x"]['value'], self.zb['CheckBox']["x"]['change']),self.coordinate.randint(self.zb['CheckBox']["y"]['value'],self.zb['CheckBox']['y']['change'])))
                logging.info(self.deviceid + u'-同意协议')
                os.system('adb -s %s shell input tap %s %s' % (self.deviceid, self.coordinate.randint(self.zb['NextStep']["x"]['value'], self.zb['NextStep']["x"]['change']), self.coordinate.randint(self.zb['NextStep']["y"]['value'], self.zb['NextStep']['y']['change'])))
                time.sleep(5)
                break
            self.Judgment_Devices()
            if self.driver.find_elements_by_id(self.element_json[u'手机号注册页面注册按钮ID']) != []:
                os.system('adb -s %s shell input tap %s %s' % (self.deviceid, self.coordinate.randint(
                    self.zb['InputRegistration']["x"]['value'], self.zb['InputRegistration']["x"]['change']),self.coordinate.randint(self.zb['InputRegistration']["y"]['value'],self.zb['InputRegistration']['y']['change'])))
            self.Judgment_Devices()
        ...
        while True:
            ...
            if self.driver.find_elements_by_class_name(self.element_json['CheckBox']) != []:
                os.system('adb -s %s shell input tap %s %s' % (self.deviceid, self.coordinate.randint(
                    self.zb['CheckBox']["x"]['value'], self.zb['CheckBox']["x"]['change']), self.coordinate.randint(
                    self.zb['CheckBox']["y"]['value'], self.zb['CheckBox']['y']['change'])))
                logging.info(self.deviceid + u'-同意协议')
                os.system('adb -s %s shell input tap %s %s' % (self.deviceid, self.coordinate.randint(
                    self.zb['NextStep']["x"]['value'], self.zb['NextStep']["x"]['change']), self.coordinate.randint(
                    self.zb['NextStep']["y"]['value'], self.zb['NextStep']['y']['change'])))
            ...
            if self.driver.find_elements_by_name('找不到网页') != []:
                ...
                self.driver.quit()
                break
            os.system('adb -s %s shell input tap %s %s' % (self.deviceid, self.coordinate.randint(self.zb['Start']["x"]['value'], self.zb['Start']["x"]['change']), self.coordinate.randint(self.zb['Start']["y"]['value'], self.zb['Start']['y']['change'])))
        ...
        if self.tm == '6':
            ...
            while True:
                try:
                    for j in range(100, 200, 30):
                        a = TouchAction(self.driver)
                        a.press(x=random.randint(240, 260), y=random.randint(990, 1010))
                        for i in range(0, 5):
                            a.move_to(x=50, y=(random.randint(-500, 0))).wait(0)
                            a.move_to(x=50, y=(random.randint(0, 500))).wait(0)
                        for i in range(0, j / 10):
    ...
    def xr_wechat(self,wxid=None,cloudCode=None,mf=None):
        ...
        if self.gj_mode == '2.神奇改机'.decode("utf-8"):
            wechat_list = '%s_%s  %s  %s  %s  dd_cloud:%s  %s\n' % (self.phonenumber[0], self.wxmm, self.ip, self.deviceid, datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'), str(self.device_token),mf)
            file().write(wechat_list,'微信账号数据.txt')
        if self.gj_mode == '1.微霸改机'.decode("utf-8"):
            wechat_list = '%s_%s  %s  %s  %s  %s  %s|  %s\n' % (self.phonenumber[0], self.wxmm, self.ip, self.deviceid, datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'), wxid,cloudCode,mf)
            file().write(wechat_list, '微信账号数据.txt')
    ...
    def send_login(self):
            ...
            if self.driver.find_elements_by_name('已发送短信，下一步')!=[]:
                os.system('adb -s %s shell input tap %s %s' % (self.deviceid, self.coordinate.randint(self.zb['SendDone']["x"]['value'], self.zb['SendDone']["x"]['change']), self.coordinate.randint(self.zb['SendDone']["y"]['value'], self.zb['SendDone']['y']['change'])))
            ...
            while True:
                ...
                if self.driver.find_elements_by_name('不是我的，继续注册')!=[]:
                    os.system('adb -s %s shell input tap %s %s' % (self.deviceid, self.coordinate.randint(self.zb['NotMine5']["x"]['value'], self.zb['NotMine5']["x"]['change']), self.coordinate.randint(self.zb['NotMine5']["y"]['value'], self.zb['NotMine5']['y']['change'])))
                    os.system('adb -s %s shell input tap %s %s' % (self.deviceid, self.coordinate.randint(self.zb['NotMine4C']["x"]['value'], self.zb['NotMine4C']["x"]['change']), self.coordinate.randint(self.zb['NotMine4C']["y"]['value'], self.zb['NotMine4C']['y']['change'])))
                self.Judgment_Devices()
                if self.driver.find_elements_by_id(self.element_json[u'输入框ID'])!=[]:
                    self.driver.quit()
                    break
                ...
                #    time.sleep(3)
                #    if self.driver.find_elements_by_id('com.dobe.sandbox:id/appIcon')!=[]:
                #        os.popen('adb -s %s shell input tap 303 480' % self.deviceid)
                #        time.sleep(5)
    ...
    def q(self,wxid):
        ...
        while True:
            ...
            if self.driver.find_elements_by_id(self.element_json[u'微信四个主按钮ID']) != []:
                ...
                os.system('adb -s %s shell input tap %s %s' % (self.deviceid, self.coordinate.randint(self.zb['Find']["x"]['value'], self.zb['Find']["x"]['change']), self.coordinate.randint(self.zb['Find']["y"]['value'], self.zb['Find']['y']['change'])))
        ...
        if self.driver.find_elements_by_name('朋友圈') != []:
            os.system('adb -s %s shell input tap %s %s' % (self.deviceid, self.coordinate.randint(self.zb['Moments']["x"]['value'], self.zb['Moments']["x"]['change']),self.coordinate.randint(self.zb['Moments']["y"]['value'], self.zb['Moments']['y']['change'])))
            time.sleep(random.randint(1, 3))
        else:
        ...
        if self.driver.find_elements_by_id(self.element_json[u'发表按钮ID']) != []:
            ...
            os.system('adb -s %s shell input tap %s %s' % (self.deviceid, self.coordinate.randint(self.zb['IKnow']["x"]['value'], self.zb['IKnow']["x"]['change']),self.coordinate.randint(self.zb['IKnow']["y"]['value'], self.zb['IKnow']['y']['change'])))
            self.input_pyq_message()
    ...
    def sandbox_save(self,wxid):
        ...
        while True:
            ...
            if len(self.driver.find_elements_by_class_name('android.widget.ImageView')) < 2:
                ...
                if 'TextView' != self.driver.find_element_by_id('com.dobe.sandbox:id/backup_info').get_attribute(("text")):
                    break
                else:
                    os.popen('adb -s %s shell rm -rf /sdcard/boxbackup'%self.deviceid)
                    self.driver.keyevent(4)
    ...
    def input_pyq_message(self):
        ...
        time.sleep(3)
        for i in ''.join([random.choice(string.ascii_lowercase) for i in range(8)]):
            os.system('adb -s %s shell input tap %s %s' % (self.deviceid, self.coordinate.randint(self.zb[i]["x"]['value'], self.zb[i]["x"]['change']),self.coordinate.randint(self.zb[i]["y"]['value'], self.zb[i]['y']['change'])))
        logging.info(self.deviceid + u'-输入文字')
        time.sleep(random.randint(1, 2))
        os.system('adb -s %s shell input tap %s %s' % (self.deviceid, self.coordinate.randint(self.zb['Push']["x"]['value'], self.zb['Push']["x"]['change']),self.coordinate.randint(self.zb['Push']["y"]['value'], self.zb['Push']['y']['change'])))
    def pd_gj(self):
        if self.gj_mode == '1.微霸改机'.decode("utf-8"):
            self.wb_new()
        if self.gj_mode == '2.神奇改机'.decode("utf-8"):
